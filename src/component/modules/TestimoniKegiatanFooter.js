...
export default function TestimoniKegiatanFooter(props) {
  ...
  return (
    <>
      <Typography variant="h4" component="h2" sx={{ mb: 3, mt: 12 }}>
        Testimoni Kegiatan
      </Typography>
        ...
        {!!result && !!result.length > 0 ? (
          result.map((tesimoni) => (
            <Grid key={tesimoni.id} item xs={12} md={6}>
              <Card
                sx={isXXS ? { display: "block" } : { display: "flex" }}
                variant="outlined"
              >
                <CardMedia
                  sx={
                ...
                <Box sx={{ display: "flex", flexDirection: "column" }}>
                  ...
                  {isAdmin && (
                    <CardActions>
                      <Link
                        href={`/admin/kegiatan/${kegiatan_id}/testimoni/${tesimoni.id}/edit`}
                        passHref
                      >
                        <Button>Edit</Button>
                      </Link>
                      <Button onClick={() => handleDelete(tesimoni.id)}>
                        Delete
                      </Button>
                    </CardActions>
                  )}
