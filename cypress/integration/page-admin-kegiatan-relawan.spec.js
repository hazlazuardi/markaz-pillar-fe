...
beforeEach(function setUser() {
  cy.visit("/", {
    onBeforeLoad(win) {
      ...
      win.localStorage.setItem(
        "currentAccessToken",
        JSON.stringify(currentAccessToken)
      );
      win.localStorage.setItem(
        "currentRefreshToken",
...
describe(`Test if all components exist and visible`, () => {
  it("Test if all exists and visible", () => {
    ...
    cy.get("[data-testid=showEntries-at-admin-or-user-template]").should(
      "exist"
    );
...
describe("Test CRUD Change Status", () => {
  it("Test if Change Status works", () => {
    ...
    cy.get("[data-testid=searchbar-at-admin-or-user-template]").type('d').clear().type(
      "rija uwu"
    );
    cy.wait(1000);
